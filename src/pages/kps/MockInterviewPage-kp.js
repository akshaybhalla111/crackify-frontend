...
function MockInterviewPage() {
  ...
  const [trialTimeLeft, setTrialTimeLeft] = useState(subscriptionStatus === 'free' ? 60 : 60);
  ...
  useEffect(() => {
    if (sessionEndedRef.current) return;  // ✅ block if already ended
    if (setupData && !isInterviewActive) {
      startInterviewWithAudio();  // ✅ safe auto start
    }
  ...
  const handleExitInterview = useCallback(async () => {
    ...
    localStorage.removeItem('mock_setup_data'); // ✅ clear persisted session
    setQuestions([]);
    setCurrentTranscript('');
    setLiveTranscriptBuffer('');
    setCurrentAnswer('');
  ...
  const handleTrialEnd = useCallback(async () => {
    ...
    await cleanup();
    localStorage.removeItem('mock_setup_data');
    setIsInterviewActive(false);
    setSetupData(null);
  ...
  const startInterviewWithAudio = async () => {
    if (sessionEndedRef.current) return; // ✅ Prevent screen share if session has ended

    await cleanup();
    setTrialTimeLeft(subscriptionStatus === 'free' ? 600 : 600); // 10 minutes trial
    sessionEndedRef.current = false;

    try {
      const stream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });

      // ✅ Immediately stop if session ended while selecting screen
